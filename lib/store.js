...
import {
  defaultConfig, defaultMilks, ensureSideFields, ensureGrowthAndMedicines,
} from './defaults.js';
...
/*
 * Deletions, kept rather than forgotten: id -> the ISO instant it happened.
 *
 * "This entry is gone" is a fact, and until now it was one the app could only
 * hold in memory. The journal recorded it, then compaction threw it away; a
 * backup could not express it at all, so restoring one brought deleted entries
 * back. On a single install that is a curiosity - the restore is a whole
 * replacement anyway. The moment two copies of the log ever have to be
 * reconciled it is data corruption: a delete on one side is invisible to the
 * other, and the entry returns.
 *
 * A tombstone is an id and a timestamp, so a thousand deletions is about sixty
 * kilobytes. They are not pruned. A cutoff would be a promise that no device
 * has been away longer than the window, and nothing here can make that promise.
 */
let deleted = new Map(); // id -> deletedAt
let timers = [];
...
function migrate(cfg) {
  ...
  if (from < 5) {
    // Per-side breastfeeding arrived: a timer banks each breast separately and
    // writes the split onto the entry, so the button needs somewhere to put it.
    // Shared with the other two editions, which cannot import this file.
    ensureSideFields(cfg);
  }
  if (from < 6) {
    // The growth button, and the medicine field turned from free text into a
    // picker of the common ones. Both additive; see ensureGrowthAndMedicines.
    ensureGrowthAndMedicines(cfg);
  }
...
function mergeConfig(stored) {
  ...
  return migrate({
    ...
    // A malformed pair here would reach the units layer, which refuses it - but
    // a non-array would break the loop that reads it, so the shape is settled
    // in the one place every config passes through.
    customUnits: Array.isArray(stored.customUnits) ? stored.customUnits : base.customUnits,
...
function applyOp(op) {
  ...
  if (op.op === 'add' && op.e && op.e.id) {
    ...
    // An add after a delete is the same id coming back - a restore replaying a
    // bundle that still has it. The tombstone stops applying.
    deleted.delete(op.e.id);
    ...
    events.delete(op.id);
    deleted.set(op.id, op.at || new Date().toISOString());
    journalOverhead += 1;
  }
}
...
/**
 * Rewrite the journal as one `add` per surviving event, plus one `del` per
 * deletion. Called on boot only.
 *
 * The tombstones are the reason this is not simply the survivors: compaction is
 * where a delete used to vanish, taking with it the only record that the entry
 * ever existed. What compaction still removes is the pair of lines a deleted
 * entry left behind (its add and its del become one del) and every superseded
 * edit, which is where the growth actually comes from.
 */
function compactIfNeeded() {
  if (journalLines < 2000 || journalOverhead < journalLines * 0.3) return;
  const body = [
    ...sortedEvents().map((e) => `${JSON.stringify({ op: 'add', e })}\n`),
    ...[...deleted].map(([id, at]) => `${JSON.stringify({ op: 'del', id, at })}\n`),
  ].join('');
  writeAtomic(EVENTS_FILE, body);
  journalLines = events.size + deleted.size;
  journalOverhead = 0;
  console.log(`[store] compacted events.log to ${events.size} entries and ${deleted.size} tombstone(s)`);
}
...
export function deleteEvent(id) {
  ...
  events.delete(id);
  deleted.set(id, at);
  // The instant matters as much as the id: reconciling two copies of the log
  // needs to know whether the delete happened before or after an edit on the
  // other side.
  appendOp({ op: 'del', id, at });
  journalOverhead += 1;
  touch();
  return true;
}

/** Every deletion this install knows about, oldest first. */
...
export function exportBundle(app = {}) {
  return {
    ...
    config,
    events: sortedEvents(),
    // What is *not* here any more, and when it went. An older release ignores
    // the field and restores exactly as it did before, which is why the format
    // version does not move: a bundle has to stay readable in both directions,
    // and a delete that is silently dropped by an old reader is no worse than
    // one that was never written down.
    deleted: listTombstones(),
    timers,
    alarmState,
  };
}
...
export function inspectBundle(bundle) {
  ...
  return {
    ...
    deleted: Array.isArray(bundle.deleted) ? bundle.deleted.filter((d) => d && d.id).length : 0,
...
export function importBundle(bundle, app = {}) {
  ...
  // Tombstones are written after the adds, so a bundle that somehow carries an
  // id in both lists resolves the way the log itself would: the delete is the
  // later fact, and it wins.
  const tombstones = (Array.isArray(bundle.deleted) ? bundle.deleted : [])
    .filter((d) => d && d.id)
    .map((d) => ({ op: 'del', id: d.id, at: d.at || new Date().toISOString() }));
  writeAtomic(EVENTS_FILE, [
    ...incoming.map((e) => `${JSON.stringify({ op: 'add', e })}\n`),
    ...tombstones.map((op) => `${JSON.stringify(op)}\n`),
  ].join(''));
