...
/**
 * Medicines and supplements a baby is commonly given at home.
 *
 * NAMES ONLY, AND DELIBERATELY SO. There is not a dose, an age or a frequency
 * anywhere in this list, and there must never be: what to give, how much, and
 * from what age is a decision for the label and the pediatrician, and an app
 * that pre-filled a number would be making it for them. This list exists so the
 * common ones are a tap instead of eleven letters typed one-handed at 4 a.m.
 *
 * Listing something is not a recommendation to give it. The list is editable
 * like every other field, and "Other" keeps anything not on it loggable.
 */
export const INFANT_MEDICINES = [
  'Vitamin D',
  'Iron',
  'Multivitamin',
  'Probiotic',
...
/**
 * Bring a config up to date with the growth button and the medicine list.
 *
 * Additive and idempotent, and shared with the editions that cannot run
 * lib/store.js, for the same reason ensureSideFields() is.
 *
 * Nothing here overwrites a choice: a medicine field somebody has already given
 * their own options keeps them, and a growth button they have edited or renamed
 * is left alone. The check is on the id, so deleting the growth button and
 * meaning it does bring it back on the next migration - the same bargain every
 * other builtin makes.
 */
export function ensureGrowthAndMedicines(config) {
  const types = config?.eventTypes;
  if (!Array.isArray(types)) return config;

  const medicine = types.find((t) => t.id === 'medicine');
  const nameField = (medicine?.fields || []).find((f) => f.key === 'name');
  // Only a field still in its shipped state is converted; one already turned
  // into a picker belongs to whoever set it up.
  if (nameField && nameField.type === 'text' && !nameField.options) {
    nameField.type = 'select';
    nameField.options = [...INFANT_MEDICINES];
    nameField.allowOther = true;
  }
  if (medicine && !(medicine.presets || []).some((x) => x.data?.name === 'Vitamin D')) {
    medicine.presets = [
      { id: 'vitd', label: 'Vitamin D', emoji: '💧', data: { name: 'Vitamin D' } },
      ...(medicine.presets || []),
    ];
  }

  if (!types.some((t) => t.id === 'growth')) {
    const at = types.findIndex((t) => t.id === 'note');
    types.splice(at >= 0 ? at : types.length, 0, growthType());
  }
  return config;
}

/**
 * Weight, length and head circumference - the three a check-up records.
 *
 * `mode: 'form'` because there is nothing to log in one tap: a measurement is
 * numbers you have to type, and a button that saved an empty one would be worse
 * than no button. Every field is `agg: 'last'`, which is what makes the charts
 * read as growth curves rather than as a month of weights added together.
 *
 * Metric, because `baby.weightKg` already is and so are the growth references
 * everyone compares against. Both units are editable, and pounds-and-ounces is
 * one field-editor change away - set the unit to `lb` and give the field a
 * smaller unit of 16.
 */
export function growthType() {
  return {
    id: 'growth',
    sound: 'ding',
    label: 'Growth',
    ...
    archived: false,
    fields: [
      { key: 'weight', label: 'Weight', type: 'number', unit: 'kg', min: 0, max: 40, step: 0.01, agg: 'last' },
      { key: 'length', label: 'Length', type: 'number', unit: 'cm', min: 0, max: 130, step: 0.1, agg: 'last' },
      { key: 'head', label: 'Head circumference', type: 'number', unit: 'cm', min: 0, max: 60, step: 0.1, agg: 'last' },
    ],
    presets: [{ id: 'measure', label: 'Measure', emoji: '📏', data: {} }],
  };
}
...
export function defaultEventTypes() {
  return [
    ...
    },
    {
      ...
      archived: false,
      fields: [
        // A picker rather than free text, so the common ones are a tap - with
        // "Other" for everything else. Names only; see INFANT_MEDICINES.
        { key: 'name', label: 'Medicine', type: 'select', options: INFANT_MEDICINES, allowOther: true },
        { key: 'dose', label: 'Dose', type: 'text' },
      ],
      presets: [
        { id: 'vitd', label: 'Vitamin D', emoji: '💧', data: { name: 'Vitamin D' } },
        { id: 'dose', label: 'Dose', emoji: '💊', data: {} },
      ],
    },
    growthType(),
    {
...
export function defaultConfig() {
  return {
    ...
    stats: {
      ...
      // Growth is a builtin, so its charts would default off like every other -
      // but a measurement button exists for its curve, so weight and length are
      // on from the start. Head circumference is off: recorded at check-ups,
      // watched by a doctor, and not something to put on a home screen unasked.
      buttons: {
        growth: { count: false, weight: true, length: true, head: false },
        // The one supplement nearly every breastfed baby is recommended, and
        // the one worth seeing a run of days for. The rest of the medicine list
        // is off until asked for, like any other builtin.
        medicine: { 'opt_name_vitamin-d': true },
      },
    ...
    // Unit pairs somebody declared for a field of their own, as
    // `{ metric, us, per, offset, metricDp, usDp }`. Empty here because the
    // five the app cannot do without - cc, ml, kg, cm, °C - are built into
    // public/js/units.js, and nothing else can be guessed: whether a "Solids"
    // button counts grams or millilitres is a fact about the person, not the
    // app.
    //
    // `per` is how many of the metric unit make one US unit, and `offset` is
    // added afterwards for a scale whose zero sits elsewhere. The metric side
    // is what every entry stores, so adding, editing or deleting a pair here
    // changes notation and never a recorded number.
    customUnits: [],
    settings: {
      ...
      volume: 0.6,
      haptics: true,
      // Metric or US. A display choice only: every value is stored in the unit
      // declared on its field and converted on the way to and from the screen,
      // so flipping this never rewrites a number. See lib/units.js.
      units: 'metric',
      refreshSeconds: 20,
